'use client';

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { ArrowLeft } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Label } from '@/components/ui/label';
import { Select } from '@/components/ui/select';
import { Textarea } from '@/components/ui/textarea';
import { FieldError } from '@/components/ui/form-section';
import { Unauthorized } from '@/components/layout/unauthorized';
import {
  InitialBalanceLineEditor,
  type InitialBalanceLineDraft,
} from '@/components/stock/initial-balance-line-editor';
import { stockErrorMessage } from '@/components/stock/stock-errors';
import { usePermissions, useWarehouses, useCreateInitialBalance } from '@/lib/auth-client';

/**
 * Separate from ordinary adjustments both in permission
 * (`inventory.initial-balance.create`) and in UX — this is only for
 * establishing the first stock a variant ever has in a warehouse, never a
 * routine correction (see docs/inventory.md). Attempting it again for a
 * variant that already has movement history is rejected server-side with
 * `INITIAL_BALANCE_ALREADY_ESTABLISHED`.
 */
export default function CargaInicialPage() {
  const router = useRouter();
  const { can, isLoading: permissionsLoading } = usePermissions();
  const warehousesQuery = useWarehouses();
  const createInitialBalance = useCreateInitialBalance();

  const [warehouseId, setWarehouseId] = useState('');
  const [notes, setNotes] = useState('');
  const [lines, setLines] = useState<InitialBalanceLineDraft[]>([]);
  const [error, setError] = useState<string | undefined>();

  if (permissionsLoading) {
    return null;
  }
  if (!can('inventory.initial-balance.create')) {
    return <Unauthorized />;
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(undefined);
    if (!warehouseId) {
      setError('Elegí un depósito.');
      return;
    }
    if (lines.length === 0) {
      setError('Agregá al menos una línea.');
      return;
    }
    if (lines.some((l) => !l.quantity || Number(l.quantity) <= 0)) {
      setError('Todas las líneas necesitan una cantidad mayor a cero.');
      return;
    }
    try {
      await createInitialBalance.mutateAsync({
        warehouseId,
        notes: notes || undefined,
        lines: lines.map((l) => ({ productVariantId: l.variantId, quantity: l.quantity })),
      });
      router.push('/stock');
    } catch (err) {
      setError(stockErrorMessage(err));
    }
  }

  return (
    <div className="flex flex-col gap-6">
      <Link
        href="/stock/ajustes"
        className="flex w-fit items-center gap-1 text-sm text-muted-foreground hover:text-foreground"
      >
        <ArrowLeft className="size-4" />
        Volver a ajustes
      </Link>

      <form onSubmit={handleSubmit} className="flex max-w-3xl flex-col gap-6" noValidate>
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Carga inicial de stock</h1>
          <p className="text-sm text-muted-foreground">
            Usá esto solo para cargar el stock inicial de un producto que todavía no tiene movimientos en este
            depósito. Si ya tiene stock y necesitás corregirlo, usá un ajuste común.
          </p>
        </div>

        <div className="flex flex-col gap-1.5 sm:max-w-xs">
          <Label htmlFor="warehouse">Depósito</Label>
          <Select id="warehouse" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} required>
            <option value="">Elegí un depósito…</option>
            {warehousesQuery.data?.warehouses
              .filter((w) => w.status === 'ACTIVE')
              .map((w) => (
                <option key={w.id} value={w.id}>
                  {w.name}
                </option>
              ))}
          </Select>
        </div>

        <div className="flex flex-col gap-1.5">
          <Label htmlFor="notes">Notas (opcional)</Label>
          <Textarea id="notes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} />
        </div>

        <div className="flex flex-col gap-1.5">
          <Label>Líneas</Label>
          <InitialBalanceLineEditor warehouseId={warehouseId || null} lines={lines} onChange={setLines} />
        </div>

        <FieldError message={error} />

        <div className="flex gap-2">
          <Button type="submit" disabled={createInitialBalance.isPending}>
            {createInitialBalance.isPending ? 'Guardando…' : 'Cargar stock inicial'}
          </Button>
        </div>
      </form>
    </div>
  );
}
