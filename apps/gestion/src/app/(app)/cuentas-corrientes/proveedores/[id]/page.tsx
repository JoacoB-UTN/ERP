...
import { supplierAccountMovementLabel, formatMoney } from '@erp/shared';
...
export default function CuentaProveedorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statementQuery = useSupplierStatement(id, effectiveCurrencyId || null);
  const openReceiptsQuery = useSupplierOpenReceipts(id, effectiveCurrencyId || null);

  if (permissionsLoading) {
    return null;
  }
  if (!can('accounts.payable.read')) {
    return <Unauthorized />;
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        title={statement?.supplier.displayName ?? 'Cuenta corriente'}
        description={statement && `${statement.supplier.code} · Cuenta corriente`}
        backHref="/cuentas-corrientes/proveedores"
      ...
      <section className="flex flex-col gap-2">
        ...
        <StatementTable
          ...
          typeLabel={supplierAccountMovementLabel}
          emptyLabel="No hay movimientos en esta moneda."
        />
      </section>

      <section className="flex flex-col gap-2">
        <h2 className="text-base font-semibold text-foreground">Recepciones pendientes de pago</h2>
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!openReceiptsQuery.isLoading && openReceipts.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">
                    No hay recepciones pendientes de pago en esta moneda.
                  </td>
                </tr>
              )}
            </tbody>
