...
import { usePermissions, useSupplierAccounts } from '@/lib/auth-client';
...
export default function CuentasProveedoresPage() {
  ...
  const accountsQuery = useSupplierAccounts({
          ...
          <tbody>
            ...
            {!accountsQuery.isLoading && !accountsQuery.isError && items.length === 0 && (
              <TableMessage
                ...
                title={search ? 'No encontramos proveedores' : 'Todavía no hay cuentas con movimientos'}
                description={
                  search
                    ? 'Probá con otro nombre o código.'
                    : 'Las cuentas aparecen acá cuando una recepción o un pago las mueve.'
                }
              />
            )}
          </tbody>
