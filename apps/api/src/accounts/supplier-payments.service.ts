...
function toSummary(
  ...
  return {
    ...
    supplier: {
      ...
      legalName: p.supplier.legalName,
    },
...
function toApplicationDto(
  a: SupplierPaymentApplication & { purchaseReceipt: { number: string } },
): SupplierPaymentApplicationDto {
  return {
    id: a.id,
...
function toDetail(
  ...
  return {
    ...
    confirmedBy: p.confirmedBy
      ? { id: p.confirmedBy, name: names.get(p.confirmedBy) ?? null }
      : null,
    cancelledAt: p.cancelledAt?.toISOString() ?? null,
    cancelledBy: p.cancelledBy
      ? { id: p.cancelledBy, name: names.get(p.cancelledBy) ?? null }
      : null,
  };
}
...
export class SupplierPaymentsService {
  ...
  async getById(
    companyId: string,
    id: string,
  ): Promise<SupplierPaymentDetailDto> {
  ...
  async update(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (rebuiltApplications) {
        await tx.supplierPaymentApplication.deleteMany({
          where: { supplierPaymentId: existing.id },
        });
  ...
  async confirm(
    ...
    if (existing.status === 'CONFIRMED')
      throw new SupplierPaymentAlreadyConfirmedException();
    if (existing.status !== 'DRAFT')
      throw new SupplierPaymentNotEditableException();
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (purchaseReceiptIds.length > 0) {
        ...
        const outstandingByReceipt =
          await this.supplierAccountService.getReceiptsOutstanding(
            tx,
            ctx.companyId,
            purchaseReceiptIds,
          );
        for (const purchaseReceiptId of purchaseReceiptIds) {
          const outstanding =
            outstandingByReceipt.get(purchaseReceiptId) ??
            new Prisma.Decimal(0);
          if (outstanding.lt(0))
            throw new SupplierPaymentOverApplicationException();
        }
      }
      ...
      await this.auditService.recordFromContext(
        ctx,
        {
          ...
          metadata: {
            ...
            number: payment.number,
            supplierName: payment.supplier.legalName,
            amount: payment.amount.toString(),
          },
        },
        tx,
  ...
  async cancel(
    ...
    if (existing.status === 'CANCELLED')
      throw new SupplierPaymentAlreadyCancelledException();
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.auditService.recordFromContext(
        ctx,
        {
          ...
          entityId: id,
          metadata: {
            change: 'confirmed_payment_cancelled',
  ...
  private async buildApplications(
    ...
    const purchaseReceiptIds = inputApplications.map(
      (a) => a.purchaseReceiptId,
    );
    const receipts = await this.prisma.purchaseReceipt.findMany({
      where: { id: { in: purchaseReceiptIds }, companyId },
    });
    ...
    for (const application of inputApplications) {
      ...
      if (!receipt || receipt.supplierId !== supplierId)
        throw new SupplierPaymentApplicationReceiptMismatchException();
      if (receipt.status !== 'CONFIRMED')
        throw new SupplierPaymentApplicationReceiptNotConfirmedException();
      if (receipt.currencyId !== currencyId)
        throw new SupplierPaymentApplicationCurrencyMismatchException();
      sumApplied = sumApplied.add(application.amount);
      built.push({ purchaseReceiptId: receipt.id, amount: application.amount });
    }
    if (sumApplied.gt(paymentAmount))
      throw new SupplierPaymentApplicationsExceedAmountException();
    ...
    for (const application of built) {
      const outstanding =
        outstandingByReceipt.get(application.purchaseReceiptId) ??
        new Prisma.Decimal(0);
